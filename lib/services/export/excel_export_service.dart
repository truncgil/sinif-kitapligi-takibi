...
class ExcelExportService {
  ...
  Future<void> exportStudentData(int studentId, String studentName) async {
    try {
      final data =
          await _databaseService.getStudentBorrowingHistoryForExport(studentId);

      if (data.isEmpty) {
        throw Exception('Bu öğrenci için dışa aktarılacak veri bulunamadı.');
      }

      await _createAndShareExcel(data, 'Librolog_$studentName');
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> exportAllData() async {
    try {
      ...
      await _createAndShareExcel(data, 'Librolog_Tum_Kayitlar');
    } catch (e) {
      rethrow;
    }
  }

  Future<void> _createAndShareExcel(
      List<Map<String, dynamic>> data, String fileNamePrefix) async {
    // 2. Excel dosyası oluştur
    var excel = Excel.createExcel();

    // Varsayılan sayfayı al veya oluştur
    String sheetName = 'Librolog Kayıtları';
    ...
    for (var row in data) {
      ...
      if (returnDateStr != null) {
        ...
        formattedReturnDate = DateFormat('dd.MM.yyyy HH:mm').format(returnDate);
      }
    ...
    // Dosya adını güvenli hale getir (boşlukları ve özel karakterleri temizle)
    final safeFileNamePrefix =
        fileNamePrefix.replaceAll(RegExp(r'[^\w\s]+'), '').replaceAll(' ', '_');
    final fileName = '${safeFileNamePrefix}_$dateStr.xlsx';
    ...
    await Share.shareXFiles(
      [XFile(path)],
      text: 'Librolog Kayıtları Excel Dosyası',
    );
  }
}
